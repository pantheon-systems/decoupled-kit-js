...
const runFnForEachNodeQuery = async fn => {
  ...
  for (const queries of chunkedQueries) {
    await Promise.all(queries.map(async queryInfo => {
      ...
      queryInfo.settings.lazyNodes && // but not in Gatsby v4+ since lazyNodes isn't supported in 4+
      !_gatsbyVersion.usingGatsbyV4OrGreater || // for media items we only want to fetch referenced nodes so don't fetch them here.
