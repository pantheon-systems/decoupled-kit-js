...
via the UI—selecting the `Pantheon Decoupled Profile` install profile. The same
can be done via
...
Visit the Site by clicking on the **Visit Development Site** button to Install
via the UI—selecting the `Demo: Umami Food Magazine` install profile. After
...
the **Front-End Sites Administration** section. Through this administration
section, the following steps should be completed to fully configure your
project:

- Edit the example preview site, or create a new preview site. See the
  instructions for
  [creating a new preview site](../../backend-starters/decoupled-drupal/configuring-preview-site).
- In the preview site list, select the **Environment Variables** action next to
  ...
  `CLIENT_SECRET` and copy the generated value. All other environment variables
  needed to configure your Front-End Site will be displayed in this modal.
