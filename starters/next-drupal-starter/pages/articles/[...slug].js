...
import { DrupalState, fetchJsonapiEndpoint } from '@pantheon-systems/drupal-kit';
...
export async function getStaticProps(context) {
  ...
  if (context?.previewData?.key) {
    ...
    const previewData = await fetchJsonapiEndpoint(`${store.apiRoot}decoupled-preview/${context.previewData.key}?include=field_media_image.field_media_image`, requestInit);
  ...
  if (context?.previewData?.resourceVersionId) {
    store.params.addCustomParam({resourceVersion: `id:${context.previewData.resourceVersionId}`});
  }
